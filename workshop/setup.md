...
6. Finally, via the `terminal`, move to the folder where you've put all the unzipped content of this workshop, e.g. with the command `cd ~/Desktop/workshop_weizmann` and run the command `jupyter notebook` from the folder that contains the `workshop_weizmann_overview.ipynb` notebook.
```
